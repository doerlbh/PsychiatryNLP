...
def main():
    ...
    if auto_adj:
        no_above = docSet.topk_dfs(topk=10)
        docSet = DocDataset(taskname,lang=lang,no_below=no_below,no_above=no_above,rebuild=rebuild,use_tfidf=False)
    ...
    with open(f'topic_dist_gsm_{taskname}.txt','w',encoding='utf-8') as wfp:
        for t,e in zip(txt_lst,embeds):
            wfp.write(f'{e}:{t}\n')
    pickle.dump({'txts':txt_lst,'embeds':embeds},open(f'gsm_embeds_{taskname}.pkl','wb'))
