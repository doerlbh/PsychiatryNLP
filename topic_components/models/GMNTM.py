...
class GMNTM:
        ...
        if (not os.path.exists('.pretrain/vade_pretrain.wght')) or retrain==True:
            ...
            for epoch in tqdm(range(pre_epoch)):
                ...
                for data in dataloader:
                    ...
                    bows_recon,_mus,_log_vars,_ = self.vade(bows,collate_fn=lambda x: F.softmax(x,dim=1),isPretrain=True)
                    #bows_recon,_mus,_log_vars,_ = self.vade(bows,collate_fn=None,isPretrain=True)
    ...
    def train(self, train_data, batch_size=256, learning_rate=2e-3, test_data=None, num_epochs=100, is_evaluate=False, log_every=5, beta=1.0, gamma=1e7,criterion='cross_entropy', ckpt=None):
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            for iter, data in enumerate(data_loader):
                ...
                bows_recon, mus, log_vars, _ = self.vade(bows,collate_fn=lambda x: F.softmax(x,dim=1),isPretrain=False)
                #bows_recon, mus, log_vars, _ = self.vade(bows,collate_fn=None,isPretrain=False)
    ...
    def get_embed(self,train_data, num=1000):
        self.vade.eval()
        data_loader = DataLoader(train_data, batch_size=16,shuffle=False, num_workers=4, collate_fn=train_data.collate_fn)
        embed_lst = []
        txt_lst = []
        cnt = 0
...
if __name__ == '__main__':
    model = VaDE(encode_dims=[1024, 16, 256, 10],
                decode_dims=[10, 128, 768, 1024])
    model = model.cuda()
