...
class VaDE(nn.Module):
    ...
    def log_pdf_gauss(self,z,mu,logvar):
        ...
        '''
            ...
            where J = latent_dim
        '''
        # output = (-0.5*(torch.sum(np.log(2*np.pi)+logvar+(z-mu).pow(2)/torch.exp(logvar),dim=1))).view(-1,1)
        output1 = np.log(2*np.pi)+logvar
        output2a = (z-mu).pow(2)
        output2 = output2a / torch.exp(logvar)
        output = -0.5*(torch.sum(output1 + output2,dim=1))
    ...
    def log_pdfs_gauss(self,z,mus,logvars):
        ...
        for c in range(self.n_clusters):
            print(mus.shape, mus[c:c+1,:].shape,c)
